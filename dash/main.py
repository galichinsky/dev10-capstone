...
def get_regional_summary():
    regional_summary = (
        df_life.groupby("region")
        .agg(avg_qol_index=("qol_index", "mean"), avg_happiness=("happiness", "mean"))
        .reset_index()
    )
...
social_support_happiness_corr = (
    df_life[["social_support", "happiness"]].corr().iloc[0, 1]
)
    ...
    return (
        html.Div(
            [
                ...
                html.H4(
                    "Correlation Matrix of Life Factors",
                    style={"textAlign": "center", "marginBottom": "-10px"},
                ),
                dcc.Graph(
                    figure=fig,
                    style={"width": "100%", "height": "1100px", "margin": "10px auto"},
                ),
            ],
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        dcc.Graph(
                            figure=fig1, style={"width": "50%", "margin": "10px"}
                        ),
                        dcc.Graph(
                            figure=fig2, style={"width": "50%", "margin": "10px"}
                        ),
                    ],
                    style={
                        "display": "flex",
...
def render_climate_tab():
    ...
    return (
        html.Div(
            [
                html.H4(
                    "Impact of Environmental Factors on Well-Being",
...
def render_social_support_tab():
    ...
    return html.Div(
        [
            html.H4(
                "Impact of Social Support on Well-Being", style={"textAlign": "center"}
            ),
            html.Div(
                [
                    dcc.Graph(
...
qol_outliers = df_life[(df_life["qol_residual"].abs() > qol_threshold)]
...
def render_outliers_tab():
    return (
        html.Div(
            [
                html.H4(
                    "Outliers in Happiness and Quality of Life",
                    style={"textAlign": "center"},
                ),
                html.Div(
                    [
                        html.H5("Happiness Outliers", style={"margin-bottom": "15px"}),
                        dash_table.DataTable(
                ...
                html.Div(
                    [
                        html.H5("Quality of Life Outliers", style={"margin-top": "20px"}),
                        dash_table.DataTable(
...
def render_qol_happiness_tab():
    return html.Div(
        [
            dcc.Graph(
                figure=fig_qol_happiness, style={"width": "90%", "margin": "auto"}
            ),
        ]
    )
...
app.layout = html.Div(
    [
        dcc.Store(id="active-tab", data="tab1-link"),  # Store the active tab
        html.H3(
            "Quality of Life and Happiness Dashboard",
            style={"textAlign": "center", "margin": "20px", "padding-left": "300px", "padding-top": "20px"},
        ),
        dbc.Row(
            [
                ...
                dbc.Col(
                    dbc.Nav(
                        [
                            ...
                            dbc.NavLink("Regional Trends", href="#", id="tab2-link"),
                            dbc.NavLink(
                                "GDP vs Quality of Life", href="#", id="tab3-link"
                            ),
                            dbc.NavLink(
                                "Climate vs QOL and Happiness", href="#", id="tab4-link"
                            ),
                            dbc.NavLink(
                                "Social Support vs QOL and Happiness",
                                href="#",
                                id="tab5-link",
                            ),
                            dbc.NavLink(
                                "Outliers in Happiness and QOL",
                                href="#",
                                id="tab6-link",
                            ),
                            dbc.NavLink("QOL vs Happiness", href="#", id="tab7-link"),
                        ],
                        vertical=True,  # Make the navigation vertical
                        pills=True,  # Use pill-style navigation
                        className="bg-light p-3",  # Add background and padding
                    ),
                    width=2,  # Set the width of the navigation column
                    style={
                        "height": "100vh",
                        "borderRight": "1px solid #ccc",
                    },  # Full height and border
                ),
                # Main content area
                dbc.Col(
                    html.Div(
                        id="tab-content",
                        style={"padding": "20px"},  # Add padding for the content
                    ),
                    width=10,  # Set the width of the content column
                ),
            ],
            style={"height": "100vh"},  # Full height for the row
        ),
    ]
)
...
@callback(
    [
        ...
        Output("tab3-link", "active"),
        Output("tab4-link", "active"),
        Output("tab5-link", "active"),
        Output("tab6-link", "active"),
        Output("tab7-link", "active"),
        Output("tab-content", "children"),
    ],
    [
        Input("tab1-link", "n_clicks"),
        Input("tab2-link", "n_clicks"),
        Input("tab3-link", "n_clicks"),
        Input("tab4-link", "n_clicks"),
        Input("tab5-link", "n_clicks"),
        Input("tab6-link", "n_clicks"),
        Input("tab7-link", "n_clicks"),
    ],
)
def update_tab_content(tab1, tab2, tab3, tab4, tab5, tab6, tab7):
    ctx = dash.callback_context
    if not ctx.triggered:
        return True, False, False, False, False, False, False, render_all_data_tab()  # Default tab

    tab_id = ctx.triggered[0]["prop_id"].split(".")[0]

    # Update active states and return the corresponding content
    return (
        tab_id == "tab1-link",
        tab_id == "tab2-link",
        tab_id == "tab3-link",
        ...
        tab_id == "tab6-link",
        tab_id == "tab7-link",
        render_all_data_tab() if tab_id == "tab1-link" else
        render_regional_tab() if tab_id == "tab2-link" else
        render_gdp_qol_tab() if tab_id == "tab3-link" else
        render_climate_tab() if tab_id == "tab4-link" else
        render_social_support_tab() if tab_id == "tab5-link" else
        render_outliers_tab() if tab_id == "tab6-link" else
        render_qol_happiness_tab(),
    )
